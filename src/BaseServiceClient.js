...
class BaseServiceClient {
    ...
    async _trainingCreateModel(address, params) {
        try {
            ...
            const ModelStateRequest =
                this._getCreateModelRequestMethodDescriptor();
            const modelStateRequest = new ModelStateRequest();
    ...
    async _trainingDeleteModel(params) {
        try {
            ...
            const ModelStateRequest =
                this._getUpdateModelRequestMethodDescriptor();
            const modelStateRequest = new ModelStateRequest();
    ...
    async loadOpenChannels() {
        try {
            const currentBlockNumber = await this.getCurrentBlockNumber();
            const newPaymentChannels =
                await this._mpeContract.getPastOpenChannels(
                    this.account,
                    this,
                    this._lastReadBlock
                );
    ...
    async updateChannelStates() {
        ...
        const currentChannelStatesPromise = this._paymentChannels.map(
    ...
    async _channelStateRequest(channelId) {
        try {
            const { currentBlockNumber, signatureBytes } =
                await this._channelStateRequestProperties(
                    toBNString(channelId)
                );
    ...
    async _fetchPaymentMetadata() {
        if (!this._options.paidCallMetadataGenerator) {
            return this._paymentChannelManagementStrategy.getPaymentMetadata(
                this
            );
        }
