...
class PaymentChannel {
     ...
     */
    async syncState() {
        logger.debug(`Syncing PaymentChannel[id: ${this._channelId}] state`, {
            tags: ['PaymentChannel'],
        });
        try {
            const latestChannelInfoOnBlockchain =
                await this._mpeContract.channels(this.channelId);
            ...
            logger.debug(
                `Latest PaymentChannel[id: ${this.channelId}] state:`,
                this._state,
                { tags: ['PaymentChannel'] }
            );
    ...
    async _currentChannelState() {
        logger.debug(
            `Fetching latest PaymentChannel[id: ${this.channelId}] state from service daemon`,
            { tags: ['PaymentChannel'] }
        );
        try {
            const response = await this._serviceClient.getChannelState(
                this.channelId
            );
            const nonce = PaymentChannel._uint8ArrayToBN(
                response.getCurrentNonce()
            );
            const currentSignedAmount = PaymentChannel._uint8ArrayToBN(
