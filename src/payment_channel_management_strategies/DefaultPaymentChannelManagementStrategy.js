    ...
    async selectChannel(serviceClient) {
        try {
            ...
            if(paymentChannels.length === 0) {
                ...
                return serviceClient.depositAndOpenChannel(
                    serviceCallPrice,
                    defaultExpiry
                );
            }
            ...
            await firstExpiredAndUnfundedChannel.extendAndAddFunds(
                defaultExpiry,
                serviceCallPrice
            );
            return firstExpiredAndUnfundedChannel;
        } catch (error) {
            throw new Error('selecting channel error: ', error);
        }
    }
    ...
    async _defaultChannelExpiry(serviceClient) {
        try {
            const currentBlockNumber =
                await this._sdkContext.web3.eth.getBlockNumber();
            return (
                currentBlockNumber +
                serviceClient.group.payment_expiration_threshold +
                this._blockOffset
            );
        } catch (error) {
            throw new Error('getting channel expiry error: ', error);
        }
    }
}
