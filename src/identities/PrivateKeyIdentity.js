    ...
    async sendTransaction(transactionObject) {
        try {
            ...
            return new Promise((resolve, reject) => {
                const method =
                    this._web3.eth.sendSignedTransaction(signedTransaction);
                method.once(
                ...
                method.on(blockChainEvents.ERROR, (errorMessage) => {
                    error(
                ...
                method.once(blockChainEvents.TRANSACTION_HASH, (hash) => {
                    info('waiting for blockchain txn', hash, {
                        tags: ['PrivateKeyIdentity'],
                    });
                });
                method.once(blockChainEvents.RECEIPT, (receipt) => {
                    info('blockchain receipt', receipt.status, {
                        tags: ['PrivateKeyIdentity'],
                    });
                });
            });
