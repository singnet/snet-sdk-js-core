...
describe('Account', () => {
    ...
    describe('_generateTokenContract', () => {
        test('should generate the token contract', async () => {
            const _tokenContract = await account._tokenContract;
        });
    });
    ...
    describe('depositToEscrowAccount', () => {
        test('should approve and deposit to the escrow account if needed', async () => {
            ...
            const receipt = await account.depositToEscrowAccount(
                new BigNumber(1000)
            );
    ...
    describe('approveTransfer', () => {
        test('should approve the specified tokens for transfer', async () => {
            ...
            expect(
                mockWeb3.eth.Contract().methods.approve
            ).toHaveBeenCalledWith('0xMpeContractAddress', '500');
    ...
    describe('allowance', () => {
        test('should fetch the already approved allowance', async () => {
            ...
            expect(
                mockWeb3.eth.Contract().methods.allowance
            ).toHaveBeenCalledWith(testWalletAddress, '0xMpeContractAddress');
